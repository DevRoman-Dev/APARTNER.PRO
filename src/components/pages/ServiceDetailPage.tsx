...
const labels = {
  ...
  uk: { whatYouGet: "Що ви отримаєте", stack: "Стек", process: "Як ми працюємо", faq: "Поширені запитання", benefits: "Чому обирають нас", startingFrom: "Від", timeline: "Терміни", rating: "на основі", reviews: "відгуків клієнтів", related: "Інші послуги", breadcrumb: { home: "Головна", services: "Послуги" } },
  ru: { whatYouGet: "Что вы получите", stack: "Стек", process: "Как мы работаем", faq: "Часто задаваемые вопросы", benefits: "Почему выбирают нас", startingFrom: "От", timeline: "Сроки", rating: "на основе", reviews: "отзывов клиентов", related: "Другие услуги", breadcrumb: { home: "Главная", services: "Услуги" } },
} as const;

export function ServiceDetailPage({ locale, service, others }: { locale: Locale; service: ServiceItem; others: ServiceItem[] }) {
    ...
    <>
          ...
          {x && (
            <div className="mt-10 flex flex-wrap items-center gap-8 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground">{l.rating} {x.rating.count} {l.reviews}</span>
              </div>
              <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
                <span className="text-foreground font-bold">{l.startingFrom}:</span> {x.startingFrom[locale]}
              </div>
              <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
                <span className="text-foreground font-bold">{l.timeline}:</span> {x.timeline[locale]}
              </div>
            </div>
          )}
        </div>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7">
            <h2 className="text-2xl font-extrabold tracking-tight mb-8 uppercase">{l.whatYouGet}</h2>
          ...
          <div className="lg:col-span-5">
            <div className="bg-secondary p-8 rounded-sm border border-border sticky top-24">
              <h3 className="text-sm font-bold uppercase tracking-widest mb-4 text-muted-foreground">{l.stack}</h3>
              <div className="flex flex-wrap gap-2 mb-8">
                {service.stack.map((s) => (
                  <span key={s} className="text-xs font-mono px-3 py-1.5 bg-background border border-border rounded-sm uppercase">{s}</span>
                ))}
              </div>
              <Link
      ...
      {x && (
        <section className="py-20 border-t border-border bg-secondary/40">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight uppercase mb-12">{l.benefits}</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {x.benefits[locale].map((b, i) => (
                <div key={b} className="p-6 bg-background border border-border rounded-sm">
                  <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-accent font-bold">0{i + 1}</span>
      ...
      {x && (
        <section className="py-20 border-t border-border">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight uppercase mb-12">{l.process}</h2>
            <ol className="grid md:grid-cols-2 gap-x-12 gap-y-8">
              {x.process[locale].map((step, i) => (
                <li key={step.title} className="flex gap-6 border-t border-border pt-6">
                  <span className="font-mono text-3xl font-extrabold text-accent shrink-0">0{i + 1}</span>
                  <div>
      ...
      {x && (
        <section className="py-20 border-t border-border bg-secondary/40">
          <div className="max-w-4xl mx-auto px-6">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight uppercase mb-12">{l.faq}</h2>
            <div className="space-y-4">
              {x.faq[locale].map((item) => (
                <details key={item.q} className="group border border-border bg-background rounded-sm p-6 [&_summary]:list-none">
      ...
      {others.length > 0 && (
        <section className="py-20 border-t border-border">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight uppercase mb-12">{l.related}</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {others.map((o) => (
                ...
                >
                  <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-accent font-bold">{o.num}</span>
                  <h3 className="text-lg font-bold mt-3 mb-2">{o.title[locale]}</h3>
                  <p className="text-sm text-muted-foreground line-clamp-2">{o.shortDesc[locale]}</p>
                </Link>
              ))}
            </div>
          </div>
